...
The source package includes the standalone APSR-P main architecture, inference, training, paired-query evaluation, MAC/RTF measurement and data preparation. `APSRP.from_pretrained()` loads the existing E102 tensor-only checkpoint strictly. The artifact verifier independently recomputes published CSV statistics without running extraction. See [integration verification](SOURCE_INTEGRATION.md) for the integration test scope.
