...
| [ESPnet / TF-GridNet](https://github.com/espnet/espnet/blob/1a753b4905932f50e7ea38012e1a289f2c3ed5d8/espnet2/enh/separator/tfgridnet_separator.py) | `models/blocks.py`: per-head attention projections and head recombination, with APSR's normalization and causal/prefix-boundary handling | [Apache-2.0](../licenses/ESPnet-Apache-2.0.txt), Johns Hopkins University (Shinji Watanabe) |

Component paths are relative to `apsr/`. The GTCRN components are configured for APSR's dimensions and conditioning. The attention changes and source references are also marked in `blocks.py`. TIGER's trainer and inference scripts are not bundled.
...
- [torch-stoi](https://github.com/mpariente/pytorch_stoi): differentiable negative eSTOI. APSR's wrapper supplies resampling and calls the upstream loss.
...
The MIT grant for APSR does not grant rights to third-party datasets or replace any dependency's terms.
